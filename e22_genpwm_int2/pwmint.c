...
__sbit __at (0xCB) GLED;
__sbit __at (0xCC) RLED;
...
static PWMstate         output_offon[NUM_PWM_CHANNELS]          = {OFF, OFF};    // current state of channel - off/on
static unsigned int     dutycount[NUM_PWM_CHANNELS]             = {0,   0};
                                                                  // ^^^ duty cycle in timer interrupt ticks, defaults to 0%
static unsigned char    desired_changed = 0;
static PWMstate         desired_output_offon[NUM_PWM_CHANNELS]  = {OFF, OFF, OFF, OFF, OFF};
static unsigned int     desired_dutycount[NUM_PWM_CHANNELS]     = {0,   0,   0,   0,   0};

static unsigned int 	timer_cnt=0; //for reset
//------------------------------------------------------------------------------------
// Timer4_PWM_Init
//------------------------------------------------------------------------------------
//
...
void Timer4_PWM_ISR (void) __interrupt 16 __using 3
{
	...
	if(timer_cnt < 10000) {++timer_cnt;}

	
	SDRPAGE = CONFIG_PAGE:

	if(timer_cnt > 2000) { GRED=0; RLED=0;}
	//2000ms (can aprox or be > that 2000?)
