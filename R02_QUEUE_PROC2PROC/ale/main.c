...
static void userRead_SW1( void *pvParameters );
static void userRead_SW2( void *pvParameters );
...
void mainInitTasks( void )
{
	...
	xTaskCreate( userRead_SW1, ( signed portCHAR * ) "READ1",  configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, ( xTaskHandle * ) NULL );
	xTaskCreate( userRead_SW2, ( signed portCHAR * ) "READ2",  configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, ( xTaskHandle * ) NULL );
...
static void userRead_SW1( void *pvParameters )
{
    ...
    for (;;)
    {
        portCHAR newstate = bioGetAB4_SW1();
        if ( newstate != state )
        {
            state = newstate;
            if (state)
...
static void userRead_SW2( void *pvParameters )
{
    ...
    for (;;)
    {
        portCHAR newstate = bioGetAB4_SW2();
        if ( newstate != state )
        {
            state = newstate;
            if (state)
    ...
    for (;;)
    {
        ...
        if ( pdTRUE == status)
        {
			...
			if(message.z == 1){
	            ...
	            if (message.value)  bioSetAB4_LED1(bioLED_ON);
	            else                bioSetAB4_LED1(bioLED_OFF);
			}
			
			if(message.z == 2){

	            if (message.value)  bioSetLEDAB4_LED2(bioLED_ON);
	            else                bioSetLEDAB4_LED2(bioLED_OFF);
			}	
        }
        else
        {
