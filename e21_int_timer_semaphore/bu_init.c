...
EA=0; // globally kill all of them

EIE1=0;
EIE2=0;
IE=0x85; // reenable global interrupts, and enable int0 and int1
CR=0; // disable the PCA

TMR3CN=TMR3CN & 0xFB; // disable timer 3
//
// slow the part down to slowest possible clockspeed in this mode
//
CLKSEL = 0x00; // Oscillator Clock Select Register ls bits 00 =
OSCICN=0x80; // sets system to internal oscliator /8
//
// ensure that all the i/o pins are off
//
P1MDIN=0x00; // running value 0xFF
//
// shut down other ports the same way
//

PCON=PCON | 0x01; // suspend the processor
//
// we come out of sleep here
//
CR=1; // enable the PCA again
EA=0;

//taken from outsourced code*
}
